...
function Skills() {
    return (
        ...
        >
            <div className="flex flex-col max-w-[1100px] w-full gap-12">
                {/* Section Header */}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {skillsCards.map((card, index) => (
                        <div
                            key={index}
                            className="group relative flex flex-col gap-5 rounded-xl border border-[#e5e7eb] dark:border-[#2a3441] bg-surface-light dark:bg-surface-dark p-6 shadow-sm hover:shadow-xl hover:shadow-primary/10 dark:hover:shadow-primary/20 transition-all duration-300 hover:-translate-y-2 overflow-hidden"
                        >
                            {/* Gradient overlay on hover */}
                            <div className="absolute inset-0 bg-gradient-to-br from-primary/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none"></div>
                            
                            <div className="relative flex items-center gap-4">
                                <div className="flex h-12 w-12 items-center justify-center rounded-lg bg-gradient-to-br from-primary/10 to-primary/5 text-primary group-hover:from-primary group-hover:to-primary/80 group-hover:text-white transition-all duration-300 shadow-sm group-hover:shadow-md group-hover:scale-110">
                                    <span className="material-symbols-outlined !text-2xl">
                                        {card.icon}
                                    </span>
                                </div>
                                <h2 className="text-[#111418] dark:text-white text-xl font-bold leading-tight group-hover:text-primary dark:group-hover:text-primary transition-colors duration-300">
                                    {card.title}
                                </h2>
                            </div>
